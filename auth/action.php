...
if (isset($_POST['login'])) {
    if (isset($_POST['email']) && isset($_POST['password'])) {
        ...
        // Validasi email
        if (empty($email)) {
            $errors['email'] = 'Silahkan masukkan email Anda.';
        }

        // Validasi password
        if (empty($password)) {
            $errors['password'] = 'Silahkan masukkan kata sandi Anda.';
        }
...
if (isset($_POST['register'])) {
    ...
    if (empty($errors)) {
        ...
        if (mysqli_query($koneksi, $insert_query)) {
            $_SESSION['success'] = "Pendaftaran berhasil, silahkan masuk.";
            echo "<script>window.location.href = '" . base_url('auth/login.php') . "';</script>";
        } else {
...
if (isset($_POST['create-admin-account'])) {
    ...
    $buat_akun      = mysqli_query($koneksi, "INSERT INTO users VALUES('$id_user', '$nama_lengkap', '$email', '$password_hash', '$level')");

    if ($buat_akun) {
        $_SESSION['success'] = "<small>Akun berhasil dibuat, silahkan masuk dengan email '$email' dan kata sandi '$password'.</small>";
    } else {
    ...
    if (isset($_GET['update-profile'])) {
        ...
        if (empty($errors)) {
            ...
            if ($_SESSION['level'] === 'Admin') {
                ...
                echo "<script>window.location.href = '" . base_url('admin/manage-user/profile.php') . "';</script>";
            } else {
