...
if (isset($_POST['create-user'])) {
    ...
    if (empty($errors)) {
        ...
        echo "<script>window.location.href = '" . base_url('admin/manage-user/role.php?role=Admin') . "';</script>";
        exit();
    } else {
        ...
        echo "<script>window.location.href = '" . base_url('admin/manage-user/create.php') . "';</script>";
        exit();
    }
}
...
if (isset($_GET['update-user'])) {
    ...
    if (empty($errors)) {
        // Update pengguna
        $update_sql = "UPDATE users SET email = '$email', nama_lengkap = '$nama_lengkap' WHERE id_user='$id_user'";
        ...
        echo "<script>window.location.href = '" . base_url('admin/manage-user/edit.php?user-id=' . $id_user) . "';</script>";
        exit();
    }
}
...
if (isset($_GET['delete-user']) && isset($_GET['current-role'])) {
    ...
    if ($query) {
        if ($id_user === $_SESSION['id_user']) {
            ...
            echo "<script>window.location.href = '" . base_url('admin/manage-user/role.php?role=' . $role_saat_ini) . "';</script>";
            exit();
        }
    } else {
