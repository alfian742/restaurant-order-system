...
<div class="pagetitle">
    <div class="d-flex justify-content-between align-items-center gap-2 mb-4">
        ...
        <div class="d-flex gap-2">
            <a href="<?= base_url('admin/order/show.php'); ?>" class="btn btn-sm px-lg-3 btn-light border-secondary">Semua</a>
            ...
            while ($data_status_pesanan = mysqli_fetch_array($query_status_pesanan)):
                if ($data_status_pesanan['status'] == 'Pending') {
                    $btn_style = 'btn btn-warning text-white';
                } elseif ($data_status_pesanan['status'] == 'Confirmed') {
                    $btn_style = 'btn btn-primary';
                } elseif ($data_status_pesanan['status'] == 'In Progress') {
                    $btn_style = 'btn btn-info text-white';
                } elseif ($data_status_pesanan['status'] == 'Completed') {
                    $btn_style = 'btn btn-success';
                } elseif ($data_status_pesanan['status'] == 'Cancelled') {
                    $btn_style = 'btn btn-danger';
                }
            ?>
                <a href="<?= base_url('admin/order/status.php?status=' . urlencode($data_status_pesanan['status'])); ?>" class="btn btn-sm px-lg-3 <?= ($status === $data_status_pesanan['status']) ? $btn_style : 'btn-light border-secondary'; ?>">
                        ...
                        <tbody>
                            ...
                            ?>
                                <tr>
                                    ...
                                    <td>
                                        <form method="post" action="<?= base_url('admin/order/action.php?update-order-status=' . $data['id_pesanan'] . '&current-status=' . $data['status']); ?>">
                                    ...
                                    <td class="text-center">
                                        <a href="<?= base_url('admin/order/detail.php?order-id=' . $data['id_pesanan']); ?>" class="btn btn-light">
